...
public class ProfileGui extends LobbyInventory {
    ...
    protected void init() {
        ...
        this.setItem(21, HEAD_ITEM.apply(UsefulHead.CRATE).withName(LobbyMessage.PROFILE_FRIENDS_NAME.get().getForPlayer(this.owner)).withLore(LobbyMessage.PROFILE_FRIENDS_LORE.get().getForPlayer(this.owner)).build(),
                e -> new FriendsGui(this.plugin, this.owner).open()
        );
        this.setItem(22, PLAYER_HEAD_ITEM.apply(this.owner.getUniqueId()).withName(LobbyMessage.PROFILE_ACCOUNT.get().getForPlayer(this.owner)).withLore(this.getAccountLore()).build());
        ...
        this.setItem(32, HEAD_ITEM.apply(UsefulHead.EARTH).withName(ChatColor.DARK_AQUA + LobbyMessage.SETTINGS_LANG.get().getForPlayer(this.owner)).withLore(LobbyMessage.PROFILE_LANGUAGE_LORE.get().getForPlayer(this.owner)).build(),
    ...
    private List<String> getAccountLore() {
        ...
        lore.add(LobbyMessage.PROFILE_RANK.get().getForPlayer(this.owner) + this.account.getPrefix());
        lore.add(ChatColor.WHITE + "Hyri+: " + (this.account.hasHyriPlus() ? ChatColor.GREEN + "✔" : ChatColor.RED + "✘"));
        lore.add(LobbyMessage.PROFILE_LEVEL.get().getForPlayer(this.owner) + this.account.getNetworkLeveling().getLevel());
        lore.add(ChatColor.WHITE + "Hyris: " + ChatColor.AQUA + this.account.getHyris().getAmount());
        lore.add(LobbyMessage.PROFILE_FIRST_JOIN.get().getForPlayer(this.owner) + DATE_FORMAT.format(this.account.getFirstLoginDate()));
        ...
        lore.add(LobbyMessage.PROFILE_PLAYTIME.get().getForPlayer(this.owner) + Duration.of(duration.toSeconds(), ChronoUnit.SECONDS).toHours() + "h " + minutes + "m");
        ...
        if (this.account.hasHyriPlus()) {
         ...
         //   lore.add(LobbyMessage.PROFILE_HYRIPLUS_EXPIRE_DATE.get().getForPlayer(this.owner) + DATE_FORMAT.format(hyriPlus.getExpirationDate()));
          //  lore.add(LobbyMessage.PROFILE_HYRIPLUS_REMAINING.get().getForPlayer(this.owner) + RandomTools.getDurationMessage(converter, this.owner));
            ...
            lore.add(LobbyMessage.PROFILE_HYRIPLUS_DONT_HAVE.get().getForPlayer(this.owner));
        }
