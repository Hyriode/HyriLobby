...
public class PlayersVisibilityLevelGui extends LobbyInventory {
    ...
    protected void init() {
        ...
        this.setupCurrentButton(HEAD_ITEM.apply(UsefulHead.ARROW_DOWN).build(), this.getSlot(), slot -> LobbyMessage.CURRENT_ITEM.asLang().getForPlayer(this.owner) + this.getLangMessage(this.level), null);
    }
    ...
    private String getLangMessage(HyriPlayersVisibilityLevel level) {
        return HyriLanguageMessage.get("settings.level." + level.name().toLowerCase()).getForPlayer(this.owner);
    }
