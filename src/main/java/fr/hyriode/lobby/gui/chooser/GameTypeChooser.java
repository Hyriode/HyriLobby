...
public class GameTypeChooser extends LobbyInventory {
    ...
    protected void init() {
        ...
        this.setItem(0, new ItemBuilder(Material.COMPASS).withName(LobbyMessage.BACK_ITEM.asLang().getForPlayer(this.owner)).build(), e -> new GamesChooserGui(this.plugin, this.owner).open());
            ...
            for (int i : this.getSlotsBySize(this.game.getGame().getTypes().size())) {
                if(this.inventory.getItem(i) == null) {
                    this.setItem(i, this.builder.clone()
                            ...
                            .withLore(LobbyMessage.LOBBY_PLAYERS_LINE.asLang().getForPlayer(this.owner) + ChatColor.AQUA + players, "", LobbyMessage.CONNECT_LINE.asLang().getForPlayer(this.owner))
    ...
    private void sendPlayerToGame(Player player, LobbyGame game, String type, String display) {
        ...
        if (hyriPlayer.isInModerationMode() || hyriPlayer.isInVanishMode()) {
            player.sendMessage(LobbyMessage.STAFF_ERROR.asLang().getForPlayer(player));
        } else if((hyriPlayer.hasParty()) && (!HyriAPI.get().getPartyManager().getParty(hyriPlayer.getParty()).isLeader(hyriPlayer.getUniqueId()))) {
            player.sendMessage(LobbyMessage.IN_PARTY_ERROR.asLang().getForPlayer(player));
        } else {
            player.sendMessage(LobbyMessage.JOIN_QUEUE_MESSAGE.asLang()
                    .getForPlayer(player)
