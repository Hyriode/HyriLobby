...
public class LobbySelectorGui extends LobbyInventory {
    ...
    protected void init() {
        ...
        final List<HyggServer> servers = HyriAPI.get().getServerManager().getLobbies();
        ...
        for (HyggServer server : servers) {
            ...
            items.put(item, e -> HyriAPI.get().getServerManager().sendPlayerToServer(this.owner.getUniqueId(), server.getName()));
        }

        this.setupPagination(servers.size(), 28, e -> this.init());
        this.tryToFill(10, this.getIndexFromPage(28), items);
    }
    ...
    private ItemStack getLobbyItem(HyggServer server) {
        ...
        return HEAD_ITEM.apply(texture).withName("Lobby ID: " + server.getName().split("-")[1]).withLore(this.getServerLore(server, friends)).build();
    }
    ...
    private List<String> getServerLore(HyggServer server, int friends) {
        ...
        lore.add(this.getMessage("connected") + server.getPlayers() + "/50");

        if (friends > 0) {
            lore.add(this.getMessage("friends") + friends);
        }

        lore.add("");
        lore.add(this.getMessage(server.getPlayers() == 50 ? "full" : "click"));
